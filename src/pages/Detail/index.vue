          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{ active: item1.isChecked * 1 === 1 }" v-for="item1 in item.spuSaleAttrValueList" :key="item1.id" @click="changeActive(item1, item.spuSaleAttrValueList)"
                >
...
export default {
  ...
  mounted() {
    ...
    // 获取action获取产品详情的信息。
    this.$store.dispatch('getGoodsList', this.$route.params.proId)
  },
  computed: {
  ...
  methods: {
    ...
    // 154.4加入购物车的回调函数 addOrUpdateShopCar
    async addShopcar() {
      // 派发请求，将产品加入到数据库(通知服务器)
      // 154.6等待着promise返回的数据
      /*  const result= await this.$store.dispatch('shopcarList',{skuId:this.$route.params.proId,skuNum:this.skuNum})
      // 服务器存储成功，接着进行路由跳转，传递参数
      //倘若失败，给用户提示
      console.log(result) */
      // 154.7换成try catch形势
      try {
        ...
        // 155.1成功之后要进行路由跳转.产品信息利用session本地存储带参。简单数据通过query传递。
